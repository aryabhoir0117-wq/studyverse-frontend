...
function initRegister() {
  ...
  p.addEventListener('input', () => {
    ...
    if (m && r.ok) m.className = 'field-msg ' + (r.strength >= 3 ? 'success' : 'warn');
    if (c.value) applyField(c, 'confirmStatus', 'confirmMsg', validateConfirm(p.value, c.value));
  });

  c.addEventListener('input', () => applyField(c, 'confirmStatus', 'confirmMsg', validateConfirm(p.value, c.value)));
    ...
    try {
      ...
      if (res.ok) {
        ...
        localStorage.setItem('user', JSON.stringify({
          ...
          xp: 0, streak: 0, lessonsCompleted: 0
        }));
        window.location.href = data.role === 'teacher' ? 'teacher-dashboard.html' : 'dashboard.html';
      } else {
  ...
  form.addEventListener('submit', async (ev) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        localStorage.setItem('user', JSON.stringify({
          username: data.username, email: data.email, role: data.role,
          xp: data.xp || 0, streak: 0, lessonsCompleted: 0
        }));
        window.location.href = data.role === 'teacher' ? 'teacher-dashboard.html' : 'dashboard.html';
      } else {
...
/* ══════════════════════════════════════════
   GOOGLE AUTH  ← only new code
══════════════════════════════════════════ */
/* ══════════════════════════════════════════
   GOOGLE AUTH
══════════════════════════════════════════ */

// Must be global and defined BEFORE GSI initializes — do NOT nest inside DOMContentLoaded
window.handleGoogleCredential = async function(response) {
  ...
  const btnOriginalText = document.getElementById(btnId)?.textContent || '';
  ...
  try {
    ...
    if (res.ok) {
      ...
      setBtn(btnId, false, btnOriginalText);
    }
  } catch {
    showError('Server error during Google sign-in. Try again.');
    setBtn(btnId, false, btnOriginalText);
  }
};

function initGoogleAuth() {
  // nothing needed here anymore — callback is already global above
}
