...
function switchTab(name) {
  ...
  document.querySelectorAll(".nav-btn").forEach(b => {
    if (b.textContent.toLowerCase().includes(name.slice(0,4)))
...
function renderTeachers() {
  ...
  tbody.innerHTML = list.map(u => {
    ...
    return `<tr>
      ...
      <td>
        <code style="font-size:11px;background:var(--bg3);padding:1px 6px;border-radius:3px">
          ${tp?.employeeId || "—"}
        </code>
      </td>
      <td style="font-size:11px;color:var(--muted);max-width:220px;line-height:1.8">
        ${assignSummary}
      </td>
      <td>
        ${u.isBlocked
...
function renderClasses() {
  ...
  if (!classes.length) {
    grid.innerHTML = `<p style="color:var(--muted)">No classes yet.</p>`;
    return;
  }
  grid.innerHTML = classes.map(c => {
    const secCount = sections.filter(
      s => String(s.classId?._id || s.classId) === String(c._id)
    ).length;
    const stuCount = allUsers.filter(
      u => String(u.studentProfile?.classId?._id || u.studentProfile?.classId) === String(c._id)
    ).length;
...
function renderSections() {
  ...
  if (!list.length) {
    grid.innerHTML = `<p style="color:var(--muted)">No sections yet.</p>`;
    return;
  }
...
window.populateSectionSelect = function (classSelectId, sectionSelectId) {
  ...
  const filtered = sections.filter(
    s => String(s.classId?._id || s.classId) === classId
  );
  secEl.innerHTML = `<option value="">Select section</option>` +
    filtered.map(s =>
      `<option value="${s._id}">Section ${s.sectionName}</option>`
    ).join("");
};

// Populate section dropdown inside a specific assignment row
function populateTaSections(rowEl) {
...
function syncAssignment(idx, field, value) {
  if (field === "subjects") {
    teacherAssignments[idx].subjects =
...
function renderAssignmentRows() {
  ...
  const rowsHTML = teacherAssignments.map((a, i) => `
    <div class="assignment-row">
      <button class="remove-row-btn" onclick="removeAssignmentRow(${i})">✕</button>

      <label>Class</label>
      <select class="ta-class-select"
        onchange="populateTaSections(this.closest('.assignment-row'));syncAssignment(${i},'classId',this.value)">
        <option value="">Select class</option>
        ${classes.map(c =>
          `<option value="${c._id}" ${a.classId === c._id ? "selected" : ""}>
            ${c.className}
          </option>`
        ).join("")}
      </select>
      ...
      <label>Section</label>
      <select class="ta-section-select"
        onchange="syncAssignment(${i},'sectionId',this.value)">
        <option value="">Select section</option>
        ${sections
          .filter(s => String(s.classId?._id || s.classId) === a.classId)
          .map(s =>
            `<option value="${s._id}" ${a.sectionId === s._id ? "selected" : ""}>
  ...
  container.innerHTML = rowsHTML + `
    <button class="add-row-btn" onclick="addAssignmentRow()">
...
function openResetModal(userId, name) {
  ...
  document.getElementById("resetPwdMsg").textContent =
    `Reset password for "${name}"? New temp password will be generated.`;
...
async function createStudent() {
  ...
  try {
    const data = await api("/api/admin/create-student", "POST",
      { username, email, classId, sectionId }
    );
    ...
    box.innerHTML = `
      ...
      <small>Share securely. Student must change on first login.</small>`;
    await loadUsers();
    renderStudents();
...
async function createTeacher() {
  ...
  if (!teacherAssignments.length) {
    toast("Add at least one class assignment", "error"); return;
  }
  // Validate all rows have class + section
  for (let i = 0; i < teacherAssignments.length; i++) {
    const a = teacherAssignments[i];
    if (!a.classId || !a.sectionId) {
      toast(`Row ${i + 1}: select both class and section`, "error"); return;
    }
  }
  try {
    ...
    box.innerHTML = `
      ...
      Temp password: <strong>${data.tempPassword}</strong>
      <small>Share securely. Teacher must change on first login.</small>`;
    await loadUsers();
    renderTeachers();
...
async function createSection() {
  ...
  if (!classId || !sectionName) {
    toast("Select class and enter section name", "error"); return;
  }
  try {
...
async function saveAssignment() {
  ...
  try {
    await api(`/api/admin/assign-student/${_assigningUserId}`, "PATCH",
      { classId, sectionId }
    );
    await loadUsers();
    renderStudents();
...
window.confirmReset = async function () {
  ...
  try {
    ...
    box.style.display = "block";
    box.innerHTML = `
      <strong>🔑 New temp password:</strong>
      <strong>${data.tempPassword}</strong>
      <small>User must change on next login.</small>`;
...
async function deactivate(userId, name) {
  if (!confirm(`Deactivate "${name}"? Soft delete.`)) return;
  try {
